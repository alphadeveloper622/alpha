...
var main = function () {
    ...
    var handleShipmentBranch = function () {
        ...
        if (document.querySelector('#shipment_branch') !== null) {
            ...
            fetch(`https://localhost:44392/api/dashboard/shipment-table?accountId=${accountId}`)
            ...
            .then(data => {
              ...
              optionsBar.xaxis.categories = data.map(item => item.City);
    ...
    var handleShipmentChart = function () {
        ...
        if (document.querySelector('#shipment_location') !== null) {
          ...
          // Fetch the data from the API endpoint with the access token in the headers
          fetch(`https://localhost:44392/api/dashboard/shipment-counts?accountId=${accountId}`, {
            headers: headers
          })
            .then(response => response.json())
            .then(data => {
              optionDonut.series = [
                data.UnderProcessingItems,
                data.DeliveredItems,
                data.ReturnedItems,
                data.UnderDistributionItems
              ];
